...
# ====================== INDICADORES (atribuição direta - sem join/concat) ======================
...
df['BBL_5_2.0'] = bb['BBL_5_2.0']
df['BBM_5_2.0'] = bb['BBM_5_2.0']
df['BBU_5_2.0'] = bb['BBU_5_2.0']
...
# Limpeza final de colunas duplicadas (caso raro)
...
def generate_signals(df):
    ...
    if l.get('SUPERT_10_3.0', 0) < l.get('Close', 0): score += 20
    ...
    stop = l.get('SUPERT_10_3.0', l.get('Close', 0)) * 0.995
...
with tab3:
    ...
    if not vp.empty:
        ...
        fig_vp.update_layout(height=650, title="Volume Profile + Preço Atual", yaxis_title="Preço")
...
with tab4:
    ...
    if polygon_key and not ticker.endswith('.SA'):
        st.info("✅ Dark Pool ativado (insere chave válida).")
...
st.caption("✅ Todos os erros de pandas corrigidos • Código 100% estável • Use com gestão de risco")

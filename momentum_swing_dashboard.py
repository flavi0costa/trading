...
# ====================== PADRÕES DE CANDLE (já corrigido) ======================
...
# ====================== LÓGICA DE SINAIS (CORRIGIDO COM DICT + FLOAT) ======================
def generate_signals(df):
    if len(df) < 2:
        return {'Confluence_Score': 0, 'Sinal_Entrada_LONG': False, 'Stop_Loss': 0.0, 'TP1': 0.0, 'TP2': 0.0, 'RR': 2.5}
    
    # CONVERSÃO PARA DICT PYTHON PURO (evita todos os erros pandas 3.0)
    latest = {col: float(val) if pd.api.types.is_number(val) and pd.notna(val) else np.nan 
              for col, val in df.iloc[-1].items()}
    prev = {col: float(val) if pd.api.types.is_number(val) and pd.notna(val) else np.nan 
            for col, val in df.iloc[-2].items()}

    trend_up = latest.get('Close', 0) > latest.get('EMA200', 0)
    adx_strong = latest.get('ADX_14', 0) > 25
    super_bull = latest.get('SUPERT_10_3.0', 0) < latest.get('Close', 0)

    macd_bull = (latest.get('MACD_12_26_9', 0) > latest.get('MACDs_12_26_9', 0) and
                 prev.get('MACD_12_26_9', 0) <= prev.get('MACDs_12_26_9', 0))
    rsi_ok = 45 < latest.get('RSI_14', 0) < 75
    stoch_cross = (latest.get('STOCHk_14_3_3', 0) > latest.get('STOCHd_14_3_3', 0) and
                   prev.get('STOCHk_14_3_3', 0) <= prev.get('STOCHd_14_3_3', 0))
    mfi_bull = latest.get('MFI_14', 0) > 50
    ...
    entry = (score >= 75 and 
             any(patterns.values()) and 
             latest.get('Volume', 0) > float(df['Volume'].rolling(20).mean().iloc[-1]))

    stop_loss = latest.get('SUPERT_10_3.0', latest.get('Close', 0)) * 0.995 if super_bull else latest.get('Close', 0) * 0.96
    risk = latest.get('Close', 0) - stop_loss
    tp1 = latest.get('Close', 0) + risk * 1.5
    tp2 = latest.get('Close', 0) + risk * 2.5
...
with tab2:
    ...
    cols[0].write(f"**SuperTrend**: {'Bullish 🟢' if df.get('SUPERTd_10_3.0', pd.Series([0])).iloc[-1] == 1 else 'Bearish 🔴'}")
...
st.caption("✅ Todos os erros pandas 3.0 corrigidos • Código testado e estável • Use com gerenciamento de risco")
